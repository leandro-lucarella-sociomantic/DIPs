...
Add a new `-checks` command-line option that selectively controls D's run-time
checks (`invariant`, `in`, `out`, `assert`, bounds checking and unexpected
`case`s in `switch`) and unifies the existing ways to customize run-time checks
...
* Similar proposal in the form of a PR by Martin Nowak:
...
An optimizing compiler is unable to eliminate calls to `_d_invariant()`
because it lacks the necessary static information. Consider:

```D
class Base { void fun(); }
class Derived : Base { invariant { ... } override void fun(); }
void caller(Base object) {
    obj.fun(); // compiler may have no static knowledge of Derived
               // so it must assume Derived may define an invariant
}
```
...
* Not all user code needs the same level of safety at the cost of performance,
  so different applications need to be able to select which run-time checks
  should be added by the compiler for different purposes
...
If `-checks=-invariant,+assert` is used, the `invariant` function will still be
...
`assert()` calls won't run, but all other code in the `debug` block will. It is
up to the user to decide which language facilities to use inside `debug` blocks.
...
those `assert()` calls won't run, but all other code in the `unittest` block
